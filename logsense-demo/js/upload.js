...
(function () {
  ...
  function wizardHtml() {
    return '<div class="wizard" id="wizard" aria-live="polite">' + STEPS.map((s, i) =>
      ...
      '<div class="wz-body"><div class="wz-title">' + s.key + '</div><div class="wz-detail"></div></div>' +
      "</div>").join("") + "</div>";
  }
  ...
  function resultHtml() {
    ...
    return '<div id="import-result" style="display:grid;gap:14px">' +
      '<div class="banner banner-good">' + icon("check") + "<div><strong>Import complete</strong>" +
      "maintenance_log_2023.xlsx processed. Records are indexed and searchable — including shorthand and Hinglish entries.</div></div>" +
      '<div class="import-result">' +
      '<div class="ir-stat"><b>' + s.detected.toLocaleString("en-IN") + "</b><span>records detected</span></div>" +
      '<div class="ir-stat"><b>' + s.usable.toLocaleString("en-IN") + "</b><span>usable</span></div>" +
      '<div class="ir-stat"><b>' + s.skipped + "</b><span>skipped (empty/duplicate)</span></div>" +
      '<div class="ir-stat"><b>' + s.review + "</b><span>require review</span></div>" +
      "</div>" +
      '<div class="flex"><a class="btn btn-primary" href="#/validation">' + icon("checksq") + "Review records</a>" +
      '<a class="btn" href="#/history">' + icon("search") + "Search the history</a></div></div>";
  }
  ...
  LS.views.upload = {
    render(host) {
      ...
      host.innerHTML =
        '<div class="page-head"><div><h1>Import Maintenance History</h1>' +
        '<p class="page-sub">Bring your existing maintenance records. No sensors. No SAP changes required.</p></div></div>' +
        ...
        "<p>or click to browse. In this demo, any drop loads the sample file.</p>" +
        ...
        '<div class="file-meta">2.4 MB · 12 sheets · sample historical log for Demo Plant</div></div>' +
        '<button class="btn btn-primary" id="btn-process">' + icon("zap") + (done ? "Re-run import demo" : "Process file") + "</button>" +
        "</div>" +
        '<div class="banner banner-info">' + icon("info") +
        "<div><strong>What LogSense reads</strong>Excel breakdown logs, CMMS/SAP exports, PDF reports, scanned handwritten registers, and technician shorthand in Hindi/English.</div></div>" +
        "</div>" +

        '<div class="card"><div class="card-head"><h2>Processing pipeline</h2><span class="card-note">simulated for this demo</span></div>' +
