...
(function () {
  ...
  LS.trustBadge = function (kind) {
    const map = {
      FACT: ["badge-fact", "database"],
      CALCULATED: ["badge-calc", "barchart"],
      HYPOTHESIS: ["badge-hypo", "alert"],
      NEW: ["badge-new", "zap"],
      REVIEW: ["badge-review", "eye"],
    };
    const [cls, ic] = map[kind] || map.FACT;
    return '<span class="tip badge ' + cls + '" tabindex="0">' + icon(ic) + LS.t("badge." + kind) +
      '<span class="tip-body">' + LS.t("badge." + kind + ".tip") + "</span></span>";
  };
  ...
  LS.statusBadge = function (status) {
    ...
    return '<span class="status-badge ' + cls + '">' + icon(ic) + LS.t("status." + status) + "</span>";
  };
  ...
  LS.openRecord = function (recordId) {
    ...
    const t = LS.t;
    LS.openDrawer(
      '<div class="drawer-head"><h2>' + t("drawer.title") + " #" + rec.id + "</h2>" +
      ...
      '<div><div class="raw-label">' + icon("file", "ic-sm") + t("drawer.original") + "</div>" +
      '<div class="raw-block">' + LS.esc(rec.raw) + "</div></div>" +
      '<div><div class="raw-label">' + icon("zap", "ic-sm") + t("drawer.normalized") + "</div>" +
      '<dl class="kv">' +
      "<dt>" + t("lbl.date") + "</dt><dd>" + LS.data.fmtDate(rec.date) + "</dd>" +
      "<dt>" + t("lbl.machine") + "</dt><dd>" + (m ? LS.esc(m.name) : rec.machineId) + "</dd>" +
      "<dt>" + t("lbl.failureMode") + "</dt><dd>" + LS.tMode(rec.mode) + "</dd>" +
      "<dt>" + t("lbl.action") + "</dt><dd>" + LS.esc(rec.normalized) + "</dd>" +
      "<dt>" + t("lbl.part") + "</dt><dd>" + (rec.part ? LS.esc(rec.part) : "—") + "</dd>" +
      "<dt>" + t("lbl.downtime") + "</dt><dd>" + (rec.downtime > 0 ? LS.data.fmtHrs(rec.downtime) : "—") + "</dd>" +
      "<dt>" + t("lbl.technician") + "</dt><dd>" + LS.esc(rec.tech) + "</dd>" +
      "<dt>" + t("lbl.source") + "</dt><dd>" + LS.esc(rec.src.f) + "</dd>" +
      (isWa ? "" : "<dt>" + t("lbl.sheet") + "</dt><dd>" + LS.esc(rec.src.s) + "</dd><dt>" + t("lbl.row") + "</dt><dd>" + LS.esc(rec.src.r) + "</dd>") +
      "<dt>" + t("lbl.recordId") + "</dt><dd class=\"mono\">#" + rec.id + "</dd>" +
      "</dl></div>" +
      (m ? '<a class="btn" href="#/machine/' + m.id + '" onclick="LS.closeDrawer()">' + icon("box") + t("drawer.open") + " " + LS.esc(m.name) + "</a>" : "") +
      "</div>"
    );
  };
  ...
  const NAV = [
    { group: null, items: [{ route: "dashboard", label: "nav.dashboard", icon: "grid" }] },
    {
      group: "grp.operations",
      items: [
        { route: "machines", label: "nav.machines", icon: "box" },
        { route: "history", label: "nav.history", icon: "clock" },
        { route: "patterns", label: "nav.patterns", icon: "activity", badge: "insights" },
        { route: "parts", label: "nav.parts", icon: "wrench" },
      ],
    },
    {
      group: "grp.ai",
      items: [
        { route: "assistant", label: "nav.assistant", icon: "chat" },
        { route: "whatsapp", label: "nav.whatsapp", icon: "phone" },
      ],
    },
    {
      group: "grp.data",
      items: [
        { route: "upload", label: "nav.upload", icon: "upload" },
        { route: "validation", label: "nav.validation", icon: "checksq", badge: "validation" },
      ],
    },
    {
      group: "grp.admin",
      items: [
        { route: "users", label: "nav.users", icon: "users" },
        { route: "settings", label: "nav.settings", icon: "sliders" },
      ],
    },
    {
      group: "grp.demo",
      items: [
        { route: "how-it-works", label: "nav.how", icon: "cpu" },
        { route: "closing", label: "nav.closing", icon: "play" },
      ],
    },
  ];
  ...
  function renderNav() {
    ...
    host.innerHTML = NAV.map((g) =>
      (g.group ? '<div class="nav-group">' + LS.t(g.group) + "</div>" : "") +
      g.items.map((it) =>
        '<a href="#/' + it.route + '" data-route="' + it.route + '">' + icon(it.icon) +
        "<span>" + LS.t(it.label) + "</span>" +
  ...
  LS.applyLang = function () {
    // static chrome
    const gsInput = document.getElementById("gsearch-input");
    if (gsInput) gsInput.placeholder = LS.t("chrome.searchPh");
    setText("demo-pill", LS.t("chrome.demoEnv"));
    setText("user-role", LS.t("role.manager"));
    // login screen
    setText("login-tagline", LS.t("login.tagline"));
    setText("login-h1", LS.t("login.signin"));
    setText("login-sub", LS.t("login.sub"));
    setText("demo-login-text", LS.t("login.demoBtn"));
    document.querySelectorAll(".lang-btn").forEach((b) =>
      b.classList.toggle("active", b.dataset.lang === LS.i18n.lang));
    const sel = document.getElementById("lang-select");
    if (sel) sel.value = LS.i18n.lang;
    // dynamic chrome + current view
    renderNav();
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // language controls
    document.getElementById("lang-select").addEventListener("change", (e) => LS.setLang(e.target.value));
    document.querySelectorAll(".lang-btn").forEach((b) =>
      b.addEventListener("click", () => LS.setLang(b.dataset.lang)));
    LS.applyLang();
